...
enum Response<T> where T: serde::Serialize {
    Result { result: T },
...
pub async fn web_server(root_logger: &LoggingOpts, args: &WebArgs) -> Result<(), CliErrors> {
    ...
    let listen_server: SocketAddr = args.listen_server
        .parse()
...
mod filters  {
    ...
    use warp::Filter;
    use super::{Db, handlers};

    pub fn get_all_users(db: Db) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    ...
    pub fn get_user_by_id(db: Db) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    ...
    pub fn get_user_by_email(db: Db) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    ...
    pub fn get_all_user_groups(db: Db) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    ...
    pub fn status() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
        warp::path!("healthz").map(|| {
            super::Response::Result { result: "OK".to_owned() }.into_response()
        })
    }
...
mod handlers {
    ...
    pub async fn get_all_user_groups(redis_server: Db) -> Result<impl warp::Reply, Infallible> {
        let result = match redis_server.get_all_user_groups().await {
            ...
            RedisResponse::Err(e) => Response::Error { message: format!("{}", e) },
    ...
    pub async fn get_user_by_id(id: String, redis_server: Db) -> Result<impl warp::Reply, Infallible> {
    ...
    pub async fn get_user_by_email(email: String, redis_server: Db) -> Result<impl warp::Reply, Infallible> {
